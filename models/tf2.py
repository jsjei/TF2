...
class TF_2(nn.Module):
    ...
    def forward(self, samples: NestedTensor, pre_imgs: NestedTensor, captions, targets):
        ...
        if not isinstance(pre_imgs, NestedTensor):
            pre_imgs = nested_tensor_from_videos_list(pre_imgs) 

        # features (list[NestedTensor]): res2 -> res5, shape of tensors is [B*T, Ci, Hi, Wi]
        # pos (list[Tensor]): shape of [B*T, C, Hi, Wi]
        features, pos = self.backbone(samples) 
        pre_imgs_features, pre_imgs_pos = self.backbone(pre_imgs) 
        ...
        # Follow Deformable-DETR, we use the last three stages outputs from backbone
        for l, (feat, pos_l) in enumerate(zip(pre_imgs_features[-3:], pre_imgs_pos[-3:])): 
            src, mask = feat.decompose()            
            src_proj_l = self.input_proj[l](src)    
            n, c, h, w = src_proj_l.shape
            ...
            pre_imgs_srcs.append(src_proj_l)
            pre_imgs_masks.append(mask)
            pre_imgs_poses.append(pos_l)
            assert mask is not None
        
        if self.num_feature_levels > (len(pre_imgs_features) - 1):
            _len_srcs = len(pre_imgs_features) - 1 # fpn level
            for l in range(_len_srcs, self.num_feature_levels):
                if l == _len_srcs:
                    src = self.input_proj[l](pre_imgs_features[-1].tensors)
                else:
                    src = self.input_proj[l](pre_imgs_srcs[-1])
                m = pre_imgs.mask
                mask = F.interpolate(m[None].float(), size=src.shape[-2:]).to(torch.bool)[0]
                pos_l = self.backbone[1](NestedTensor(src, mask)).to(src.dtype)
                n, c, h, w = src.shape
        ...
        hs, memory, init_reference, inter_references, enc_outputs_class, enc_outputs_coord_unact, inter_samples = \
                                            self.transformer(srcs, pre_imgs_srcs, text_embed, masks, pre_imgs_masks, poses, pre_imgs_poses, query_embeds)
        ...
        for lvl in range(hs.shape[0]):
            ...
            # print("boxes:", out['pred_boxes'], "lvl_references:", lvl_references)
