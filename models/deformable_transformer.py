...
class DeformableTransformer(nn.Module):
    ...
    def forward(self, srcs, pre_imgs_srcs, tgt, masks, pre_imgs_masks, pos_embeds, pre_imgs_pos_embeds, query_embed=None):
        ...
        spatial_shapes = []
        for lvl, (src, mask, pos_embed) in enumerate(zip(pre_imgs_srcs, pre_imgs_masks, pre_imgs_pos_embeds)):
            bs, c, h, w = src.shape
            spatial_shape = (h, w)
        ...
        valid_ratios = torch.stack([self.get_valid_ratio(m) for m in pre_imgs_masks], 1)

        # encoder
        # memory: [bs*t, \sigma(hi*wi), c]
        pre_imgs_memory = self.encoder(src_flatten, spatial_shapes, level_start_index, valid_ratios, lvl_pos_embed_flatten, mask_flatten)
        pre_imgs_spatial_shapes, pre_imgs_level_start_index, pre_imgs_valid_ratios, pre_imgs_mask_flatten = spatial_shapes, level_start_index, valid_ratios, mask_flatten
        ...
        if self.two_stage:
            ...
            # reference_points = ref_pts.unsqueeze(0).repeat(b*t, 1, 1).sigmoid()
            init_reference_out = reference_points

        # decoder
        hs, inter_references, inter_samples = self.decoder(tgt, reference_points, memory, pre_imgs_memory, 
                                            spatial_shapes, pre_imgs_spatial_shapes, level_start_index, pre_imgs_level_start_index, valid_ratios, pre_imgs_valid_ratios, query_embed, mask_flatten, pre_imgs_mask_flatten)
...
class DeformableTransformerDecoderLayer(nn.Module):
    ...
    def forward(self, tgt, query_pos, reference_points, src, pre_imgs_src, src_spatial_shapes, pre_imgs_src_spatial_shapes, level_start_index, pre_imgs_level_start_index, src_padding_mask=None, pre_imgs_src_padding_mask=None):
        # self attention
        ...
        tgt2, sampling_locations, attention_weights = self.cross_attn(self.with_pos_embed(tgt, query_pos),
                               reference_points,
                               pre_imgs_src, pre_imgs_src_spatial_shapes, pre_imgs_level_start_index, pre_imgs_src_padding_mask)
...
class DeformableTransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, reference_points, src, pre_imgs_src, src_spatial_shapes, pre_imgs_src_spatial_shapes, src_level_start_index, pre_imgs_src_level_start_index, src_valid_ratios, pre_imgs_src_valid_ratios, 
                query_pos=None, src_padding_mask=None, pre_imgs_src_padding_mask=None):
        ...
        for lid, layer in enumerate(self.layers):
            ...
            output, sampling_locations, attention_weights = layer(output, query_pos, reference_points_input, 
                                                        src, pre_imgs_src, src_spatial_shapes, pre_imgs_src_spatial_shapes, src_level_start_index, pre_imgs_src_level_start_index, src_padding_mask, pre_imgs_src_padding_mask)
