...
class YTVOSDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        while not instance_check:
            ...
            imgs, pre_imgs, labels, boxes, masks, valid = [], [], [], [], [], []
            for j in range(self.num_frames):
                ...
                # get pre_boxes
                length = len(frames)
                if frame_indx == length-1:
                    pre_frame_indx = frame_indx - 1
                elif frame_indx % 2 == 0:
                    pre_frame_indx = frame_indx + 1
                else:
                    pre_frame_indx = frame_indx - 1
                pre_frame_name = frames[pre_frame_indx]
                
                # read pre_img and pre_mask
                pre_img_path = os.path.join(str(self.img_folder), 'JPEGImages', video, pre_frame_name + '.jpg')
                pre_mask_path = os.path.join(str(self.img_folder), 'Annotations', video, pre_frame_name + '.png')
                ...
                pre_mask = ((pre_mask==obj_id)).astype(np.float32) # 0,1 binary
                # get the target mask           
                ...
                # pre_img.save("pre_img_" + frame_name + ".png")
            ...
            imgs, target_1 = self._transforms(imgs, target_0) 
            imgs = torch.stack(imgs, dim=0) # [T, 3, H, W]
            
            pre_imgs, _ = self._transforms(pre_imgs, target_0) 
            pre_imgs = torch.stack(pre_imgs, dim=0) # [T, 3, H, W]
            
            
            # FIXME: handle "valid", since some box may be removed due to random crop
            if torch.any(target_1['valid'] == 1):  # at leatst one instance
                instance_check = True
            else:
                idx = random.randint(0, self.__len__() - 1)

        return imgs, pre_imgs, target_1
